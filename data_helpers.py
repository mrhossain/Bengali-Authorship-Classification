...
def get_unicode(input):
    #input = unicode(input, errors='replace')
    #print input
    input=chomps(input)
    if type(input) != str: #unicode:
...
def get_datasets_localdata(container_path=None, categories=None, load_content=True,encoding='utf-8', shuffle=True, random_state=42):
    ...
    print("load_content",load_content)
    print("encoding",encoding)
    print("shuffle",shuffle)
    print("random_state",random_state)
    datasets = load_files(container_path=container_path, categories=categories,load_content=load_content, shuffle=shuffle, encoding=encoding,random_state=random_state)
    #print(datasets)
    return datasets
...
def load_data_labels(datasets):
    ...
    print("LEN",len(x_text))
    #clean_str(x).split(' ')).tolist()
...
def load_embedding_vectors_word2vec(vocabulary, filename, binary):
    #print("........I Am Here",vocabulary, " ",filename, " ",binary)
...
def load_embedding_vectors_glove(vocabulary, filename, vector_size):
    print(len(vocabulary),vector_size,filename)
...
def load_embedding_vectors_fasttext(vocabulary, filename, vector_size):
    print(len(vocabulary),vector_size,filename)
    # load embedding_vectors from the glove
    # initial matrix with random uniform
